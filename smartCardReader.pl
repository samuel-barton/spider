...
#
#              The user is asked for their login credentials, and is given two
#              chances to correctly enter his/her user name, and three for his
#              or her password. He or she is then required to enter a message
#              describing the purpose of his or her visit to the workspace.
#              Upon entering a purpose to their visit, the login is logged and
#              the user is added to the hash of currently logged in users.
#
#              Logout:
#
...
#              - ask for a user id and password
...
until ($stop)
{
    ...
    my $result = &isAuthorizedUser($data);
    ...
    if (defined $logged_in_users{$result})
    {
        # aquire the time they logged in.
        my $old_time = $logged_in_users{$result};

        # log the user out.
        delete $logged_in_users{$result};
        ...
        system("clear");
        say "$result, you have been successfully logged out. Good bye.";
        sleep(2);
    }
    # if the user is not logged in and the id is recognized
    elsif ($result ne $UNAUTHORIZED_CARD)
    {        
        ...
        # if the user's id does not match the card number, let the user retype 
        # one time, then log an error and make them restart the process.
        if ($result ne $user_name)
        {
            print "login: ";
            $user_name = <STDIN>;
            chomp($user_name);
            ...
            if ($result ne $user_name)
            {
                ...
                &logError("user name does not match card number.");
                $restart = 1;
            }
        }
        ...
        unless ($restart)
        {
            ...
            # give the user two more chances to enter their password, and if
            # they fail to enter it properly both times, then go back to the
            # top of the loop and log an unsuccessful login attempt (user name
            # and pwd mismatch)
            if ($passwords{$user_name} ne $password)
            {
                # ask the user for their password a second time (2 tries down,
                # one more before restart)
                print "\npassword: ";
                $password = <STDIN>;
                chomp($password);

                # if the password does not match the user name (final chance)
                if ($passwords{$user_name} ne $password)
                {
                    print "\npassword: ";
                    $password = <STDIN>;
